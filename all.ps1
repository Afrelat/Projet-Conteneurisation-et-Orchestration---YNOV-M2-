...
Write-Host "Attente de la disponibilité des APIs (Identity / Jobs / Applicants / WebMVC)..." -ForegroundColor Gray
kubectl wait --for=condition=ready pod -l app=identity-api -n $NAMESPACE --timeout=180s
kubectl wait --for=condition=ready pod -l app=jobs-api -n $NAMESPACE --timeout=180s
kubectl wait --for=condition=ready pod -l app=applicants-api -n $NAMESPACE --timeout=180s
kubectl wait --for=condition=ready pod -l app=webmvc -n $NAMESPACE --timeout=180s
